...
// }
  ...
  constructor(private el: ElementRef) {
    ...
    ngAfterViewInit() {
    const parent = this.el.nativeElement.parentNode;
    const label = parent.querySelector('label'); // Find associated label if any
    ...
    if (label) {
      parent.insertBefore(span, label.nextSibling); // insert after label
    } else {
      parent.appendChild(span); // fallback
    }
  }
