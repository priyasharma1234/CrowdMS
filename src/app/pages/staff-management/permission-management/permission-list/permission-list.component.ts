...
export class PermissionListComponent implements OnInit {
  ...
  constructor(private _apiRequestService: ApiRequestService, private toaster: NgxToasterService,
    public permissionService: PermissionService,
    public permissionServiceService: PermissionServiceService,
    private _activatedRoute: ActivatedRoute) {
    // this.headerService.sendHeader('Permission-List');
  }
  ...
  async handleDelete(permission: any) {
    ...
    console.log("In delete functionn and ", this.deleteMode, "with permission ", permission);
    if (!this.deleteMode) return;
    let res: any = await this._apiRequestService.postDataAsync({ name: permission.name, guard_name: this.userType }, apiRoutes.permissions.removePermission);
  ...
  private async getPermissionList() {
    let res = await this._apiRequestService.postDataAsync({ guard_name: this.userType }, apiRoutes.permissions.permissionList);
    if (res.statuscode == 200) {
      this.permissions = res.data;
      this.permissions.forEach((per: any) => per.permissions = per.permission.map((t: any) => {
        let temp = t;
        return { nameDisplay: temp.replace(per.module + "-", ""), enabled: false, name: t }
      }));
  ...
  async AddPermission() {
    let data = await this._apiRequestService.postDataAsync({ name: this.permissionToAdd, guard_name: this.userType }, apiRoutes.permissions.permissionCreate);
  ...
  async AddCustomPermission() {
    let data = await this._apiRequestService.postDataAsync({ ...this.customPermission, guard_name: this.userType }, apiRoutes.permissions.customPermissionCreate);
    if (data.statuscode == 200) {
      this.toaster.showSuccess("Permission Added Successfully", "Success");
      await this.getPermissionList();
      this.isCustom = false;
