...
export class PermissionListComponent implements OnInit {
    ...
    async handleDelete(permission: any) {
        ...
        let res: any = await this._apiRequestService.postDataAsync({ name: permission.name, guard_name: 'admin' }, apiRoutes.permissions.removePermission);
    ...
    private async getPermissionList() {
        let res = await this._apiRequestService.postDataAsync({ guard_name: 'admin' }, apiRoutes.permissions.permissionList);
        if (res.statuscode == 200) {
    ...
    async AddPermission() {
        let data = await this._apiRequestService.postDataAsync({ name: this.permissionToAdd, guard_name: 'admin' }, apiRoutes.permissions.permissionCreate);
    ...
    async AddCustomPermission() {
        let data = await this._apiRequestService.postDataAsync({ ...this.customPermission, guard_name: 'admin' }, apiRoutes.permissions.customPermissionCreate);
        if (data.statuscode == 200) {
            this.toaster.showSuccess("Permission Added Successfully", "Success");
            await this.getPermissionList();
            this.isCustom = false;
